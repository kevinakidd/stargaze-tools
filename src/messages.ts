...
function getStringField(object: any, key: string | number): string {
  ...
  if (typeof value !== 'string') {
    throw new Error(
      `Wrong type for key '${key}' in object: ${typeof value}. Must be a string.`
    );
  }
  return value;
}
...
function getArrayField(object: any, key: string | number): any[] {
  ...
  if (!Array.isArray(value)) {
    throw new Error(
      `Wrong type for key '${key}' in object: ${typeof value}. Must be an array.`
    );
  }
  return value;
}

function importMsgExecuteContractEncodeObject(
  object: unknown
...
function importMsgInstantiateContract(
  object: unknown
